    ...
    for dataframe in fontes:
        ...
        for _, linha in dataframe.iterrows():
            ...
            registros.append({
                ...
                "Classificação interna": internal_classification(linha),
...
def mostrar_candidatos_prioritarios(metricas_df: pd.DataFrame, fontes: list[pd.DataFrame]) -> None:
    ...
    cards = [
        ("Gerados", formatar_contagem(n_gerados), "candidatos", IMAGENS_ETAPAS_FUNIL[0]),
        ("Viáveis", formatar_contagem(n_viaveis), "candidatos", IMAGENS_ETAPAS_FUNIL[1]),
        ("Refinados", formatar_contagem(n_refinados), "candidatos", IMAGENS_ETAPAS_FUNIL[2]),
        ("Finais (Top 10)", formatar_contagem(n_finais), "candidatos", IMAGENS_ETAPAS_FUNIL[3]),
    ]
    cards_html = []
    for rotulo, valor, nota, caminho_imagem in cards:
        imagem_base64 = base64.b64encode(caminho_imagem.read_bytes()).decode("utf-8") if caminho_imagem.exists() else ""
        imagem_html = (
            f"<img src='data:image/png;base64,{imagem_base64}' alt='{html.escape(rotulo)}'>"
            if imagem_base64
            else ""
        )
        cards_html.append(
            f"<div class='candidate-metric'><div class='candidate-metric-image'>{imagem_html}</div>"
            f"<div class='candidate-metric-copy'><b>{html.escape(rotulo)}</b><strong>{html.escape(valor)}</strong>"
            f"<span>{html.escape(nota)}</span></div></div>"
        )
    st.markdown(f"<div class='candidate-metrics'>{''.join(cards_html)}</div>", unsafe_allow_html=True)
    ...
    for posicao, (_, linha) in enumerate(candidatos_df.iterrows(), start=1):
        ...
        confianca = linha["Classificação interna"]
        classe_confianca = {"alta": "high", "media": "medium", "baixa": "low"}.get(normalizar_texto(confianca), "unknown")
    ...
    st.markdown("<div class='candidate-results-layout'><div class='candidate-table-wrap'><table class='candidate-table'><thead><tr>" "<th>#</th><th>Fórmula</th><th>Suporte sugerido</th>" "<th>Estabilidade termodinâmica<br>(eV/átomo) ↓</th><th>Pontuação final<br>(0-1) ↑</th>" "<th>Incerteza<br>(desvio MC)</th><th>Rota de síntese</th><th>Classificação<br>interna</th><th>Composição do score<br>(0-1)</th>" f"</tr></thead><tbody>{tabela_html}</tbody></table></div>" "<aside class='candidate-mcda-panel'><h4>Composição do score (MCDA)</h4>" "<div class='candidate-donut'><span>PESOS<br>(%)</span></div>" "<div class='candidate-mcda-item'><b>Atividade (40%)</b><span>Desempenho catalítico previsto (0-1)</span></div>" "<div class='candidate-mcda-item'><b>Estabilidade (30%)</b><span>Estabilidade termodinâmica (eV/átomo; mais negativa é melhor)</span></div>" "<div class='candidate-mcda-item'><b>Seletividade (20%)</b><span>Seletividade para o produto-alvo (0-1)</span></div>" "<div class='candidate-mcda-item'><b>Estabilidade operacional (10%)</b><span>Consistência diante de variações estruturais e operacionais (0-1)</span></div>" "<p>Score final: soma ponderada normalizada entre 0 e 1.</p></aside></div>" "<div class='candidate-legend'><span>↓ Valores mais negativos indicam maior estabilidade termodinâmica.</span>" "<span>↑ Valores mais altos indicam melhor desempenho global.</span>" "<span><i class='high'></i> Alta &nbsp; <i class='medium'></i> Média &nbsp; <i class='low'></i> Baixa</span><span>Classificação interna não calibrada; não representa probabilidade de acerto nem validação experimental.</span></div>", unsafe_allow_html=True)
...
def aplicar_estilo_interface() -> None:
    ...
    st.markdown(
        ...
        <style>
            ...
            .candidate-metric { display: grid; grid-template-columns: 68px minmax(0, 1fr) 68px; min-height: 112px; align-items: center; gap: 12px; padding: 14px; border: 1px solid #DCE6EE; border-radius: 8px; background: #FFFFFF; box-shadow: 0 3px 10px rgba(20, 33, 61, 0.04); }
            .candidate-metric::after { content: ''; display: block; width: 68px; height: 1px; }
            .candidate-metric-image { display: flex; align-items: center; justify-content: center; width: 68px; height: 68px; border-radius: 8px; background: #F4F8FC; overflow: hidden; }
            .candidate-metric-image img { display: block; width: 64px; height: 64px; object-fit: contain; }
            .candidate-metric-copy { display: flex; flex-direction: column; align-items: center; justify-content: center; min-width: 0; width: 100%; text-align: center; }
            .candidate-metric b { display: block; color: #14213D; font-size: 0.8rem; text-align: center; } .candidate-metric strong { display: block; color: #087A3B; font-size: 1.55rem; line-height: 1.1; margin-top: 7px; text-align: center; } .candidate-metric span { color: #64748B; font-size: 0.76rem; text-align: center; }
